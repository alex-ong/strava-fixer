...
def download_gpx(strava: StravaWeb, activity_id: int, path: Path) -> None:
    gpx, extension = strava.get_gpx(activity_id)
    print ("Downloaded file with extension:", extension)
    filename = Path(path, str(activity_id) + extension)
    tmpfilename = Path(path, str(activity_id) + extension + ".tmp")
...
def download_activities(db: sqlite3.Connection, strava: StravaWeb, dir_activities: Path) -> None:
    new = 0    
    processed = 0
    for activity in db.execute("SELECT id FROM activity WHERE upload_id IS NOT NULL ORDER BY id DESC"):
        activity_id = int(activity['id'])
        print(activity_id)
        if find_gpx(dir_activities, activity_id):
            processed += 1
