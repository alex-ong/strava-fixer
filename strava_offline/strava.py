...
class StravaWeb:
    ...
    def _get_gpx(self, what: str, activity_id: int) -> (bytes, str):
        ...
        content_disposition_ok = (
            content_disposition == "attachment" and
            self.valid_extension(content_disposition_params['filename']))

        if content_type_ok and content_disposition_ok:
            return (r.content, content_disposition_params['filename'][-4:])
        else:
            raise NotGpx(f"expected gpx attachment, got:\n{r.headers}")

    def get_gpx(self, activity_id: int) -> (bytes, str):
        try:
